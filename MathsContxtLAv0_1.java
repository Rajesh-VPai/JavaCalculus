    ...
    public static LookAheadVar parseVar(String str, Map<String, Double> variables, String DiffWithRespTo) {
        ...
        return new Object() {
            ...
            LookAheadVar parseVar() {
                ...
                if ((!FunctionStr.equalsIgnoreCase("")) && (!x.eval().equalsIgnoreCase("1")) && (!x.eval().equalsIgnoreCase(DiffWithRespTo)) && (!x.eval().contains(DiffWithRespTo + "^")) && (!x.eval().contains("("))) {
...
//                }
                ...
                for (;;) {
                    if ((eatVar('*') || (TermOperatorPos == -1) || (StrVar.charAt(TermOperatorPos) == '*'))) { // multiplication
                        ...
                        if (TermOperatorPos == -1) {
                            ...
                            if (!a.eval().equalsIgnoreCase("0.0")) {
                                x = (() -> (a.eval() + "*" + b.eval()));
                            } else {
                                x = (() -> b.eval());
                            }
            ...
            LookAheadVar parseBase() {
                ...
                if ((IsNumber(chVar))) { //numbers
                    ...
                    //nextCharVar();
                    int startPosPara = posVar;
                    while (chVar != ')') {
                        nextCharVar();
                    }
                    String name = StrVar.substring(startPosPara, posVar + 1).trim();
                    log.info("LookAheadVar:parseBase:Variable:Independent Constant:name=" + name + " x.eval()=" + x.eval() + " ContainsIndConstFlag=" + ContainsIndConstFlag);
                    final String MyStr = name;
                    x = (() -> MyStr);
                    log.info("LookAheadVar:parseBase:Variable:Independent Constant:name=" + name + " x.eval()=" + x.eval() + " ContainsIndConstFlag=" + ContainsIndConstFlag);
...
//                }
            ...
            SimpleAlgebraHghLvl parseTermSimple() {
                ...
                pos=-1;
                for (c = 0;;) {
                    v = parseSimpleTerm(StrFunF.substring(posFunF), variables, DiffWithRespTo);
                    TermF = v.SimpleTerm();
                    StrAlgeTermFunF[c++] = TermF;
                    log.fatal("AlgebraHghLvl:parseTermSimple:Primary Loop parseTermSimple:x.SimpleExpr()=" + x.Algebra() + " pos=" + pos + " ch=" + (char) ch + " posTerm=" + posTerm + " chTerm=" + (char) chTerm);
                ...
                pos=-1;
                for (d = 0;;) {
                    u = parseSimpleTerm(StrFunG.substring(posFunG), variables, DiffWithRespTo);
                    TermG = u.SimpleTerm();
...
//                }
            ...
            FoGLowLvL parseTermSimple() {
                ...
                pos=-1;
                for (c = 0;;) {
                    v = parseSimpleTerm(StrFunG.substring(posFunG), variables, DiffWithRespTo);
                    TermF1 = v.SimpleTerm();
...
//                        }
    ...
    public static DiffExpr ApplyFuncRules(String AlgoName, String Term1, DiffExpr Diff1, String Term2, DiffExpr Diff2, String Term3, String Term4, String StrOperator, String Rule, String DiffWithRespTo, int TermOperator) {
        ...
        if (Rule.equalsIgnoreCase("NoExponentRuleTerm")) {
            ...
            if (((Term1 != null) && (!Term1.equalsIgnoreCase("")))
                ...
                if (!Diff2DiffExpr.equalsIgnoreCase(DiffWithRespTo)) {
            ...
            if ((!Diff1DiffExpr.equalsIgnoreCase("")) && (!Diff2DiffExpr.equalsIgnoreCase("")) && (!Diff2DiffExpr.equalsIgnoreCase("0.0"))) {
                ...
                log.info("Diff:ApplyFuncRules:Else:Term2=" + Term2 + " Diff2DiffExpr=" + Diff2DiffExpr + " Diff1.DiffExpr=" + Diff1DiffExpr);
                x = (() -> Diff1DiffExpr);
            }
    ...
    public static IntegrExpr ApplyFuncRules(String AlgoName, String Term1, IntegrExpr Diff1, String Term2, IntegrExpr Diff2, String Term3, String Term4, String StrOperator, String Rule, String DiffWithRespTo, int TermOperator) {
        ...
        if (Rule.equalsIgnoreCase("NoExponentRuleTerm")) {
            ...
            if (((Term1 != null) && (!Term1.equalsIgnoreCase("")))
                ...
                if (Term3.equalsIgnoreCase("")) {
                    ...
                    if ((Term1.equalsIgnoreCase(Diff2IntegrExpr)) && (Term1.equalsIgnoreCase(DiffWithRespTo))) {
                        if (IsAConstant(Term3, DiffWithRespTo)) {
                            x = (() -> Diff1IntegrExpr + Term3 + StrOperator + Diff2IntegrExpr + "^2.0*1.0/2.0");
                            log.info("Integr:ApplyFuncRules:ConstantRule :Rule2.3.1.A :x.IntegrExpr()=" + x.IntegrExpr());
                        } else {
                            x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr + "^2.0*1.0/2.0");
                            log.info("Integr:ApplyFuncRules:ConstantRule :Rule2.3.1.B :x.IntegrExpr()=" + x.IntegrExpr());
                        }
            ...
            } else if ((IsAConstant(Diff1IntegrExpr, DiffWithRespTo)) || ((!IsAConstant(Diff1IntegrExpr, DiffWithRespTo)) && (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))) && (IsAConstant(Diff2IntegrExpr, DiffWithRespTo))) {
                x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr);
                ...
                || (Rule.equalsIgnoreCase("SubtractionRule"))) {
            if ((LookAheadOp[loadoppos] != LowLevelOperator.End) && (!Diff1IntegrExpr.equalsIgnoreCase("")) && (!Diff2IntegrExpr.equalsIgnoreCase("")) && (!Diff2IntegrExpr.equalsIgnoreCase(Term2)) && (Term3.equalsIgnoreCase(""))) {
                if (Diff2IntegrExpr.contains(DiffWithRespTo)) {
                    x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr);
                    log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule1.1 :x.IntegrExpr()=" + x.IntegrExpr() + " loadoppos=" + loadoppos);
                } else {
                    x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr + "*" + DiffWithRespTo);
                    log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule1.2 :x.IntegrExpr()=" + x.IntegrExpr() + " loadoppos=" + loadoppos);
                }
                log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule1 :x.IntegrExpr()=" + x.IntegrExpr() + " loadoppos=" + loadoppos);
            } else if ((LookAheadOp[loadoppos] != LowLevelOperator.End) && (!Term1.equalsIgnoreCase("")) && (!Term2.equalsIgnoreCase("")) && (!Diff2IntegrExpr.equalsIgnoreCase(Term2)) && (Term3.equalsIgnoreCase(""))) {
                x = (() -> Term1 + StrOperator + Term2);
                log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule2 :x.IntegrExpr()=" + x.IntegrExpr());
            } else if ((LookAheadOp[loadoppos] != LowLevelOperator.End) && (!Diff2IntegrExpr.equalsIgnoreCase(Term2)) && (Term3.equalsIgnoreCase(""))) {
                x = (() -> Diff1IntegrExpr);
                ...
                if ((Diff2IntegrExpr.equalsIgnoreCase(Term2)) && (Term2.contains(DiffWithRespTo)) && (!Term3.contains(DiffWithRespTo + "^"))) {
                    x = (() -> Diff1IntegrExpr + StrOperator + Term2 + "^2.0" + "*1/2.0");
            ...
            } else if ((LookAheadOp[loadoppos] == LowLevelOperator.End) && (!IsAConstant(Diff2IntegrExpr, DiffWithRespTo)) && ((StrIsAConstant.equalsIgnoreCase("IndConstantExpression")) || (StrIsAConstant.equalsIgnoreCase("VarExpression")))) {
                if (Diff2IntegrExpr.equalsIgnoreCase(Term2 + "*" + DiffWithRespTo)) {
                    ...
                    if ((IsAConstant(Diff2IntegrExpr, DiffWithRespTo)) || ((!IsAConstant(Diff2IntegrExpr, DiffWithRespTo)) && ((StrIsAConstant.equalsIgnoreCase("IndConstant")) || (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))))) {
                        x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr + "*" + DiffWithRespTo);
                        log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule6D.1 :x.IntegrExpr()=" + x.IntegrExpr());
                    } else {
                        x = (() -> Diff1IntegrExpr + StrOperator + Diff2IntegrExpr);
                        log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule6D.2 :x.IntegrExpr()=" + x.IntegrExpr());
                    }
                ...
                if ((IsAConstant(Term2, DiffWithRespTo)) || ((!IsAConstant(Term2, DiffWithRespTo)) && ((StrIsAConstant.equalsIgnoreCase("IndConstant")) || (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))))) {
                    ...
                    log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule7A.1 :x.IntegrExpr()=" + x.IntegrExpr());
                } else {
                    x = (() -> Term1 + StrOperator + Term2);
                    log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule7A.2 :x.IntegrExpr()=" + x.IntegrExpr());
                }
            ...
            if ((IsAConstant(Term3, DiffWithRespTo)) && (StrIsAConstant.equalsIgnoreCase("Number"))) {
                if ((LookAheadOp[j] == LowLevelOperator.Mult) && (Diff2IntegrExpr.contains("Constant") && (Double.parseDouble(Term3) > 2.0))) {
                ...
                } else if ((LookAheadOp[j] == LowLevelOperator.Div) && (Diff2IntegrExpr.contains("Constant") && (Double.parseDouble(Term3) > 2.0))) {
                    x = (() -> "-1.0/" + RemoveEDbl(-Double.parseDouble(Term3) + 1) + "*" + Term2 + "^" + RemoveEDbl(-Double.parseDouble(Term3) + 1));
                    log.info("Integr:ApplyFuncRules:ExponentRule2:x.IntegrExpr()=" + x.IntegrExpr());
                } else if ((Diff2IntegrExpr.contains("Parenthesis")) && (Double.parseDouble(Term3) > 2.0)) {
                    x = (() -> "1.0/" + RemoveEDbl(Double.parseDouble(Term3) + 1) + "*" + Term2 + "^" + RemoveEDbl(Double.parseDouble(Term3) + 1));
            ...
            if (Supported == 1) {
                if ((!Diff1IntegrExpr.equalsIgnoreCase(Diff2IntegrExpr)) && (StrOperator.equalsIgnoreCase("*"))) {
                    ...
                    if (Diff1IntegrExpr.contains("Parenthesis")) {
                        Expr_Func_Split(Diff1IntegrExpr, DiffWithRespTo);
                        Ux = ParanExpr;
                        UDashx = ParanDiffExpr;
                    ...
                    if (Diff2IntegrExpr.contains("Parenthesis")) {
                        Expr_Func_Split(Diff2IntegrExpr, DiffWithRespTo);
                        Vx = ParanExpr;
                        VDashx = ParanDiffExpr;
                ...
                if ((((IsAConstant(Diff1IntegrExpr, DiffWithRespTo))
                    ...
                    if (Diff2IntegrExpr.startsWith("1.0")) {
                        ...
                        if ((Diff1IntegrExpr.endsWith("/")) && (Term1.startsWith("/"))) {
                            ...
                            if ((!Term1.equalsIgnoreCase("")) && (!Diff1IntegrExpr.contains(Term1))) {
                                x = (() -> Diff1IntegrExpr + Term1 + Temp);
                                log.info("Integr:ApplyFuncRules:Rule5B.3.A :x.IntegrExpr()=" + x.IntegrExpr());
                            } else {
                                x = (() -> Diff1IntegrExpr + Temp);
                                log.info("Integr:ApplyFuncRules:Rule5B.3.B :x.IntegrExpr()=" + x.IntegrExpr());
                            }
...
//                }
    ...
    public static String ApplyFuncRulesString(String AlgoName, String Term1, IntegrExpr Diff1, String Term2, IntegrExpr Diff2, String Term3, String Term4, String StrOperator, String Rule, String DiffWithRespTo, int TermOperator) {
        ...
        if (Rule.equalsIgnoreCase("NoExponentRuleTerm")) {
                ...
                if (Term3.equalsIgnoreCase("")) {
                    ...
                    if ((Term1.equalsIgnoreCase(Diff2.IntegrExpr())) && (Term1.equalsIgnoreCase(DiffWithRespTo))) {
                        if (IsAConstant(Term3, DiffWithRespTo)) {
                            ...
                            log.info("Integr:ApplyFuncRulesString:ConstantRule :Rule2.3.1.A :x=" + x);
                        } else {
                            x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr() + "^2.0*1.0/2.0";
                            log.info("Integr:ApplyFuncRulesString:ConstantRule :Rule2.3.1.B :x=" + x);
                        }
                    } else {
            ...
            } else if ((IsAConstant(Diff1.IntegrExpr(), DiffWithRespTo)) || ((!IsAConstant(Diff1.IntegrExpr(), DiffWithRespTo)) && (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))) && (IsAConstant(Diff2.IntegrExpr(), DiffWithRespTo))) {
                x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr();
            ...
            if ((LookAheadOp[loadoppos] != LowLevelOperator.End) && (!Diff1.IntegrExpr().equalsIgnoreCase("")) && (!Diff2.IntegrExpr().equalsIgnoreCase("")) && (!Diff2.IntegrExpr().equalsIgnoreCase(Term2)) && (Term3.equalsIgnoreCase(""))) {
                if (Diff2.IntegrExpr().contains(DiffWithRespTo)) {
                    x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr();
                    log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule1.1 :x=" + x + " loadoppos=" + loadoppos);
                } else {
                    x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr() + "*" + DiffWithRespTo;
                    log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule1.2 :x=" + x + " loadoppos=" + loadoppos);
                }
                ...
                if (Diff2.IntegrExpr().equalsIgnoreCase(Term2 + "*" + DiffWithRespTo)) {
                    ...
                    if ((IsAConstant(Diff2.IntegrExpr(), DiffWithRespTo)) || ((!IsAConstant(Diff2.IntegrExpr(), DiffWithRespTo)) && ((StrIsAConstant.equalsIgnoreCase("IndConstant")) || (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))))) {
                        x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr() + "*" + DiffWithRespTo;
                        log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule6D.1 :x=" + x);
                    } else {
                        x = Diff1.IntegrExpr() + StrOperator + Diff2.IntegrExpr();
                        log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule6D.2 :x=" + x);
                    }
                    log.info("Integr:ApplyFuncRules:SumRule or SubtractionRule :Rule6D :x=" + x);
                }
                log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule6 :x=" + x);
            } else if ((LookAheadOp[loadoppos] == LowLevelOperator.End) && (!Term1.equalsIgnoreCase("")) && (!Term2.equalsIgnoreCase(""))) {
                if ((IsAConstant(Term2, DiffWithRespTo)) || ((!IsAConstant(Term2, DiffWithRespTo)) && ((StrIsAConstant.equalsIgnoreCase("IndConstant")) || (StrIsAConstant.equalsIgnoreCase("IndConstantExpression"))))) {
                    x = Term1 + StrOperator + Term2 + "*" + DiffWithRespTo;
                    log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule7A.1 :x=" + x);
                } else {
                    x = Term1 + StrOperator + Term2;
                    log.info("Integr:ApplyFuncRulesString:SumRule or SubtractionRule :Rule7A.2 :x=" + x);
                }
                ...
                if ((((IsAConstant(Diff1.IntegrExpr(), DiffWithRespTo))
                    ...
                    if (Diff2.IntegrExpr().startsWith("1.0")) {
                        ...
                        if ((Diff1.IntegrExpr().endsWith("/")) && (Term1.startsWith("/"))) {
                            ...
                            //String Temp1 = Diff1.IntegrExpr().replaceFirst("/", "");
                            if ((!Term1.equalsIgnoreCase("")) && (!Diff1.IntegrExpr().contains(Term1))) {
                                x = Diff1.IntegrExpr() + Term1 + Temp;
                                log.info("Integr:ApplyFuncRulesString:Rule5B.3.A :x=" + x);
                            } else {
                                x = Diff1.IntegrExpr() + Temp;
                                log.info("Integr:ApplyFuncRulesString:Rule5B.3.B :x=" + x);
                            }
                        }
                    } else {
    ...
    public static String FormSimpleTermParse(String AlgoName, String IndConstStr, String BracesStr, String VarStr, String ConstStr, String DiffWithRespTo, boolean BracesFlag, boolean VariableFlag, boolean ConstantFlag, boolean IndConstFlag, int TermOperator, int TermOperatorBraces, int TermOperatorIndConst, int TermVarOperator) {
        ...
        if ((VariableFlag == true) && ((VarStr.startsWith("\\*")) || (TermOperator == '*') || (TermOperator == -1)) && (VarStr.contains(DiffWithRespTo))
            ...
            if ((ConstStr.equalsIgnoreCase("1.0")) && (LAVarTermOperator != '/')) {
                if ((ConstStr.equalsIgnoreCase("1.0")) && (LAIndConstOperator == '*')) {
                    CompleteStr = CompleteStr + VarStr;
                    log.info("SimpleTerm:FormSimpleTermParse:VarConstantRules:Rule1.1A:CompleteStr=" + CompleteStr);
                } else if ((ConstStr.equalsIgnoreCase("1.0")) && (LAIndConstOperator == '/')) {
                    CompleteStr = IndConstStr + (char) LAVarTermOperator + VarStr;
                    log.info("SimpleTerm:FormSimpleTermParse:VarConstantRules:Rule1.1B:CompleteStr=" + CompleteStr);
                } else if ((LATermOperator == -1) && (LAVarTermOperator == '*') && (VariableFlag == true) && (ConstantFlag == true)) {
                    if ((ConstStr.equalsIgnoreCase("1.0"))) {
                        CompleteStr = CompleteStr + VarStr;
                        log.info("SimpleTerm:FormSimpleTermParse:VarConstantRules:Rule1.1C:CompleteStr=" + CompleteStr);
                    }
                }
                log.info("SimpleTerm:FormSimpleTermParse:Normal:Rule1:CompleteStr=" + CompleteStr);
            ...
            } else if ((ConstantFlag == true) && (VariableFlag == true) && (IndConstFlag == false) && (!ConstStr.equalsIgnoreCase("0.0")) && (!VarStr.equalsIgnoreCase("0.0")) && (!VarStr.equalsIgnoreCase(""))) {
                CompleteStr = CompleteStr + (ConstStr + (char) TermOperator + VarStr);
                log.info("SimpleTerm:FormSimpleTermParse:End:Rule11.7:Final:CompleteStr=" + CompleteStr);
            } else if ((ConstantFlag == true) && (VariableFlag == true) && (IndConstFlag == true) && (!ConstStr.equalsIgnoreCase("0.0")) && (!VarStr.equalsIgnoreCase("0.0")) && (!VarStr.equalsIgnoreCase(""))) {
                CompleteStr = CompleteStr;
                log.info("SimpleTerm:FormSimpleTermParse:End:Rule11.8:Final:CompleteStr=" + CompleteStr);
            }
